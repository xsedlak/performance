...
# this is the custom test sequence runner, uncomment parts of it to start tests:
# then start it with 
# $ nohup ./tests.sh &> tests.log &
# you can put the STOP file in current dir to gracefullly shutdown the test run (this can take time)
...
Z11=${JJ_HOME}/zing-11.zvm-22.01.2.0-1.product

#latest dev of Mar.03.2022
#Z11=${JJ_HOME}/zing-11.zvm-dev-3744.product


Z8=${JJ_HOME}/zing20.08.0.0-4-jdk8.0.265

OJDK11=${JJ_HOME}/openjdk-11.0.2
#OJDK11=${JJ_HOME}/openjdk-11+28
...
export YCSB_JAVA_HOME=/home/rscherba/ws/zing20.08.0.0-4-jdk8.0.265-linux_x64 # important! tlp-stress doesn't work with other builds
...
CC_HOST="10.21.21.231:31111"


#CNC+COCA options from Ivan Sidorkin
CNC_COCA_OPTS="\
-XX:+UnlockExperimentalVMOptions -XX:+UnlockDiagnosticVMOptions \
-XX:UseKlassNameBasedKID=4190000 -XX:ProfileLogOut=rnProfile.log \
-XX:+UseCNC  -XX:CNCHost=10.21.20.229:31453 -XX:+CNCAbortOnBadChannel \
-XX:-CNCLocalFallback \
-Xlog:concomp=info:file=connected-compiler-client.log -XX:+CNCInsecure \
-XX:CNCDebugOptions=+codecache.read,+codecache.write \
"

#CNC with disabled COCA options  from Ivan Sidorkin
CNC_OPTS="\
-XX:+UnlockExperimentalVMOptions -XX:+UnlockDiagnosticVMOptions \
-XX:UseKlassNameBasedKID=4190000 -XX:ProfileLogOut=rnProfile.log \
-XX:+UseCNC  -XX:CNCHost=10.21.16.218:31967 -XX:+CNCAbortOnBadChannel \
-Xlog:concomp=info:file=connected-compiler-client.log -XX:+CNCInsecure \
"
...
do
for target in 110k
#for target in 20k 30k 40k 50k 60k 70k 80k 90k
#for target in 20k 30k 40k 50k 60k 70k 80k 90k 100k 105k 110k 120k
#for target in 50k 70k 110k 130k
#for target in 80k 90k 100k 105k 110k 120k
#for target in 40k
do
for dat in 1
do
...
#export CASSANDRA_PROPS=num_tokens=8,concurrent_compactors=2,compaction_throughput_mb_per_sec=100
...
#TEST=${WL}//target=${target},threads=${threads},time=${t},rtw=15m
#TEST=${WL}//target=${target},threads=${threads},time=${t},rtw=40m
#TEST=${WL}//target=${target},threads=${threads},time=${t},rtw=5m
#long warmup
TEST=${WL}//target=${target},threads=${threads},time=${t},rtw=25m
...
./run.sh  JAVA_OPTS="-Xms${heap}g -Xmx${heap}g ${UNLOCK_XX} ${ZING_VM_LOG} ${CNC_OPTS} __LOGGC__ __LOGCOMP__ ${ZING_OPTS}"  ${TEST}
...
# ZULU ZGC
export STAMP=$(date -u '+%Y%m%d_%H%M%S')_${WL}_zulu11_h${heap}_zgc_${suff}
#./run.sh  JAVA_OPTS="-Xms${heap}g -Xmx${heap}g ${ZGC} __LOGGC__"  JAVA_HOME=$ZULU_11  JAVA_VERSION=11  $TEST
check_stopped

# ZULU SHND
export STAMP=$(date -u '+%Y%m%d_%H%M%S')_${WL}_zulu11_h${heap}_shnd_${suff}
#./run.sh  JAVA_OPTS="-Xms${heap}g -Xmx${heap}g ${SHND} __LOGGC__"  JAVA_HOME=$ZULU_11  JAVA_VERSION=11  $TEST
check_stopped
